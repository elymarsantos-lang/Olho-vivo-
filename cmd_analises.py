...
- IA principal: Perplexity (via OpenAI SDK).
...
# OpenAI (SDK) – usado para falar com a API do Perplexity (base_url customizada)
try:
...
# =====================================================================
# 🔑 CHAVE DA PERPLEXITY (API KEY)
# =====================================================================
OPENAI_API_KEY = os.getenv(
    "OPENAI_API_KEY",
    "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",
)
...
def fig_to_base64(fig: Figure) -> str:
    """
    Gera PNG em memória e retorna string base64 para HTML.
    """
    import io
    buf = io.BytesIO()
...
def normalize_cols(df: pd.DataFrame) -> pd.DataFrame:
    """
    VERSÃO MELHORADA: Remove acentos, espaços e deixa minúsculo.
    Ex: 'Valor (R$)' -> 'valor_r' | 'Depósito' -> 'deposito'
    """
    new_cols = []
...
def find_col(df: pd.DataFrame, candidates: List[str]) -> Optional[str]:
    """
    Versão robusta: tenta match exato e depois substring.
    """
...
# -----------------------------
# NOVA FUNÇÃO DE GRÁFICOS (ADICIONADA AQUI)
# -----------------------------
def exibir_graficos_games(insights: dict):
    """
    Gera janelas com gráficos dos Top Jogos (Rollback e Freespin).
    Deve ser chamada logo após 'detect_alerts_and_insights'.
    """
    sns.set_theme(style="whitegrid")
    
    # Gráfico 1: Rollback
    if "top_rollbacks_por_brand_e_jogo" in insights:
        dados = insights["top_rollbacks_por_brand_e_jogo"]
        if dados:
            df_rb = pd.DataFrame(dados)
            plt.figure(figsize=(12, 6))
            sns.barplot(data=df_rb.head(15), x="sum", y="game", hue="brand", dodge=False)
            plt.title("TOP Jogos - Maior Rollback (Prejuízo)")
            plt.xlabel("Total (R$)")
            plt.ylabel("Jogo")
            plt.legend(title="Marca")
            plt.tight_layout()
            plt.show() # Se estiver usando Streamlit, use st.pyplot(plt)

    # Gráfico 2: Freespin
    if "top_freespins_por_brand_e_jogo" in insights:
        dados_fs = insights["top_freespins_por_brand_e_jogo"]
        if dados_fs:
            df_fs = pd.DataFrame(dados_fs)
            plt.figure(figsize=(12, 6))
            sns.barplot(data=df_fs.head(15), x="sum", y="game", hue="brand", dodge=False)
            plt.title("TOP Jogos - Maior Custo de Freespin")
            plt.xlabel("Total (R$)")
            plt.ylabel("Jogo")
            plt.legend(title="Marca")
            plt.tight_layout()
...
def detect_alerts_and_insights(df: pd.DataFrame, cfg: dict, filename: str) -> Tuple[List[Alert], Dict[str, Any]]:
    import re # Garante que o regex funcione
    ...
    # --- MICRO-FUNÇÃO DE LIMPEZA (Interna para não quebrar) ---
    def _clean_money_internal(val):
        """Limpa valores monetários (ex: 'R$ 10.00' ou 'Sem Rollback')"""
        s_val = str(val)
        if "Sem " in s_val or s_val.strip() in ["-", "", "nan", "None"]:
            return 0.0
    ...
    # --- APLICAR LIMPEZA ---
    # Isso transforma o texto "R$ 0.04" em número 0.04
    if rollback_val_col:
        df[rollback_val_col] = df[rollback_val_col].apply(_clean_money_internal)
    
    if freespin_val_col:
        df[freespin_val_col] = df[freespin_val_col].apply(_clean_money_internal)
    ...
    if game_col: 
        df[game_col] = df[game_col].fillna("Desconhecido").astype(str)
        # Remove hífens isolados nos nomes dos jogos
        df.loc[df[game_col] == '-', game_col] = 'Desconhecido'
    ...
    # --- 2. INSIGHTS POR BRAND (ROLLBACK E FREESPIN) ---
    if brand_col:
        # 2.1 Visão Geral (Soma simples)
        if rollback_val_col: # Já foi limpo acima, é numérico agora
        ...
        # 2.2 Visão Detalhada: Brand + Jogo
        if game_col:
            # Rollback
            if rollback_val_col:
                # Filtra zeros para processar mais rápido
                df_rb = df[df[rollback_val_col] > 0]
                if not df_rb.empty:
                    rb_game_stats = df_rb.groupby([brand_col, game_col])[rollback_val_col].agg(['sum', 'count']).reset_index()
                    rb_game_stats.columns = ['brand', 'game', 'sum', 'count'] # Padroniza chaves
                    rb_game_stats = rb_game_stats.sort_values(by='sum', ascending=False).head(50)
                    insights["top_rollbacks_por_brand_e_jogo"] = rb_game_stats.to_dict(orient='records')

            # Freespin
            if freespin_val_col:
                df_fs = df[df[freespin_val_col] > 0]
                if not df_fs.empty:
                    fs_game_stats = df_fs.groupby([brand_col, game_col])[freespin_val_col].agg(['sum', 'count']).reset_index()
                    fs_game_stats.columns = ['brand', 'game', 'sum', 'count'] # Padroniza chaves
                    fs_game_stats = fs_game_stats.sort_values(by='sum', ascending=False).head(50)
                    insights["top_freespins_por_brand_e_jogo"] = fs_game_stats.to_dict(orient='records')
...
def analise_universal_agnostica(df: pd.DataFrame) -> Dict[str, Any]:
    """
    Lê TODAS as colunas e gera estatísticas, não importa o nome.
    Separa em Dimensões (Textos) e Métricas (Números).
    """
    stats = {
        "dimensoes_identificadas": [],
    ...
    for col in df.columns:
        # Pula colunas com muitos valores únicos (provavelmente IDs ou Hashes)
        if df[col].nunique() > (len(df) * 0.95) and len(df) > 50:
            continue
            
        if pd.api.types.is_numeric_dtype(df[col]):
            cols_numericas.append(col)
            stats["metricas_identificadas"].append({
                "coluna": col,
                "soma": float(df[col].fillna(0).sum()),
                "media": float(df[col].fillna(0).mean())
            })
        else:
            # Se tiver menos de 50 opções únicas, consideramos uma Categoria (Ex: Status, Brand, Jogo)
            if df[col].nunique() < 50:
                cols_categoricas.append(col)
                stats["dimensoes_identificadas"].append(col)

    # 2. Cruzamento Automático (Categoria x Métrica)
    # Pega as top 5 categorias e cruza com as top 3 métricas
    for cat in cols_categoricas[:5]:
        for num in cols_numericas[:3]:
            try:
                # Agrupa: Soma de [Numero] por [Categoria]
                agrupado = df.groupby(cat)[num].sum().sort_values(ascending=False).head(5)
                stats["correlacoes_auto"].append({
                    "analise": f"Top {cat} por {num}",
                    "dados": agrupado.to_dict()
                })
            except:
                continue
...
def exibir_graficos_games(insights):
    """Desenha os gráficos de Rollback e Freespin por Jogo."""
    sns.set_theme(style="whitegrid")
    ...
    if "top_rollbacks_por_brand_e_jogo" in insights:
        ...
        if dados:
            ...
            sns.barplot(data=df_rb.head(15), x="sum", y="game", hue="brand", dodge=False)
            plt.title("TOP Jogos com Maior Rollback (Prejuízo)")
    ...
    if "top_freespins_por_brand_e_jogo" in insights:
        ...
        if dados_fs:
            ...
            plt.figure(figsize=(10, 6))
            sns.barplot(data=df_fs.head(15), x="sum", y="game", hue="brand", dodge=False)
            plt.title("TOP Jogos com Maior Freespin (Custo)")
            plt.xlabel("Valor Total (R$)")
            plt.ylabel("Jogo")
            plt.legend(title="Marca")
            plt.tight_layout()
...
def make_charts(
    ...
    """
    Gera gráficos STACKED (Empilhados) por Brand.
    - Universal: Para colunas gerais (Status, Processadora).
    - Específico: Força a geração de Top Jogos (Rollback/Freespin) e Tipos de KYC.
    """
    charts = []
    ...
    sns.set_theme(style="dark", rc={
        "axes.facecolor": "#121829", "figure.facecolor": "#0B0F19", "text.color": "#E6E9EF",
        "axes.labelcolor": "#E6E9EF", "xtick.color": "#E6E9EF", "ytick.color": "#E6E9EF",
        "grid.color": "#222A3A", "axes.edgecolor": "#222A3A"
    })

    # --- PALETA DE MARCAS (IGUAL AO ORIGINAL) ---
    BASE_COLORS = {
        '7k': '#A0C62E', 'cassino': '#2F78D7', 'vera': '#40DF4A',
        'br4bet': '#FF9F43', 'outros': '#95A5A6', 'indefinido': '#57606f'
    }

    def get_brand_palette(unique_brands):
        pal = {}
        for b in unique_brands:
            s = str(b).lower().strip().replace(".0", "")
            if '7k' in s: pal[b] = BASE_COLORS['7k']
            elif 'cassino' in s: pal[b] = BASE_COLORS['cassino']
            elif 'vera' in s: pal[b] = BASE_COLORS['vera']
            elif 'br4bet' in s: pal[b] = BASE_COLORS['br4bet']
            else: pal[b] = BASE_COLORS['outros']
        return pal
    ...
    # Brand (Fundamental para o Stacked)
    brand_col = find_col(['brand', 'casa', 'marca', 'site'])
    if not brand_col:
        df['_brand_gen'] = 'Geral'
        brand_col = '_brand_gen'
    
    df[brand_col] = df[brand_col].fillna("Indefinido").astype(str).str.replace(".0", "", regex=False)

    # Colunas Específicas (Jogos e KYC)
    game_col = find_col(['game', 'jogo', 'slot', 'game_name'])
    kyc_col = find_col(['operation', 'tipo', 'type', 'step', 'etapa']) # Procura Onboarding/Liveness aqui

    # Métricas Específicas (Rollback e Freespin)
    rb_val_col = find_col(['rollback_valor', 'rollback_amount', 'valor_rollback'])
    fs_val_col = find_col(['freespin_valor', 'freespin_amount', 'valor_freespin', 'bonus_amount'])
    
    # Se não achar valor, tenta achar contagem
    rb_qtd_col = find_col(['rollback_count', 'qtd_rollback', 'rollback'])
    fs_qtd_col = find_col(['freespin_count', 'qtd_freespin', 'freespin'])

    # --- 2. GRÁFICOS UNIVERSAIS (CATEGORIAS GERAIS) ---
    # Ignora colunas com muitas variações (como ID ou Jogo), mas processa Status, Motivos, etc.
    cat_cols = [c for c in df.columns if df[c].dtype == 'object' and 1 < df[c].nunique() < 50 and c not in [brand_col, game_col]]
    
    for col in cat_cols[:10]:
        try:
            top_vals = df[col].value_counts().head(15).index
            df_plot = df[df[col].isin(top_vals)].copy()
            if df_plot.empty: continue

            fig, ax = plt.subplots(figsize=(10, 5))
            sns.histplot(data=df_plot, y=col, hue=brand_col, multiple="stack",
                         palette=get_brand_palette(df_plot[brand_col].unique()),
                         edgecolor="#0B0F19", linewidth=0.5, shrink=0.8, ax=ax)
            ...
            charts.append({"title": f"Análise: {col}", "img_b64": fig_to_base64(fig)})
        except: pass

    # --- 3. BLOCO ESPECÍFICO: TOP JOGOS (ROLLBACK E FREESPIN) ---
    # Aqui forçamos a análise de jogos, mesmo que tenha 1000 jogos (pegamos Top 15)
    if game_col:
        analises_games = [
            {"tipo": "Rollback", "col_val": rb_val_col, "col_qtd": rb_qtd_col},
            {"tipo": "Freespin", "col_val": fs_val_col, "col_qtd": fs_qtd_col}
        ]

        for item in analises_games:
            # Decide qual métrica usar: Valor ($) é prioridade, se não tiver usa Qtd
            metrica = item["col_val"] if item["col_val"] else item["col_qtd"]
            label_metrica = "Valor (R$)" if item["col_val"] else "Ocorrências"
            
            if metrica and pd.api.types.is_numeric_dtype(df[metrica]):
                try:
                    # Filtra apenas onde tem valor > 0
                    df_games = df[df[metrica] > 0].copy()
                    if df_games.empty: continue

                    # Pega os Top 15 Jogos
                    top_games = df_games.groupby(game_col)[metrica].sum().nlargest(15).index
                    df_plot = df_games[df_games[game_col].isin(top_games)].copy()

                    if not df_plot.empty:
                        fig, ax = plt.subplots(figsize=(11, 6))
                        
                        # Plota usando weights para somar o valor financeiro ou quantidade
                        sns.histplot(
                            data=df_plot, y=game_col, weights=metrica,
                            hue=brand_col, multiple="stack",
                            palette=get_brand_palette(df_plot[brand_col].unique()),
                            edgecolor="#0B0F19", linewidth=0.5, shrink=0.8, ax=ax
                        )

                        # Labels inteligentes (K/M se for dinheiro)
                        for container in ax.containers:
                            labels = []
                            for v in container.datavalues:
                                if v == 0: labels.append('')
                                elif item["col_val"] and v > 1000: labels.append(f'{v/1000:.0f}K') # Dinheiro
                                else: labels.append(f'{int(v)}') # Qtd
                            ax.bar_label(container, labels=labels, label_type='center', color='white', fontsize=7.5, weight='bold')

                        ax.set_title(f"Top 15 Jogos - {item['tipo']} ({label_metrica})", color='white', weight='bold', loc='left')
                        ax.set_xlabel(label_metrica)
                        ax.set_ylabel("")
                        sns.despine(left=True)
                        sns.move_legend(ax, "lower right", bbox_to_anchor=(1, 1), frameon=False, title=None, ncol=3)

                        charts.append({"title": f"Jogos: {item['tipo']}", "img_b64": fig_to_base64(fig)})
                except: pass

    # --- 4. BLOCO ESPECÍFICO: KYC (Liveness / Onboarding) ---
    # Força um gráfico para a coluna que indica o tipo de operação, se encontrada
    if kyc_col:
        try:
            # Verifica se essa coluna já não foi plotada no loop universal
            if kyc_col not in cat_cols[:10]: 
                fig, ax = plt.subplots(figsize=(10, 5))
                sns.histplot(data=df, y=kyc_col, hue=brand_col, multiple="stack",
                             palette=get_brand_palette(df[brand_col].unique()),
                             edgecolor="#0B0F19", linewidth=0.5, shrink=0.8, ax=ax)
                ...
                ax.set_title(f"Distribuição KYC: {kyc_col} (Onboarding/Liveness)", color='white', weight='bold', loc='left')
                ax.set_ylabel("")
                sns.despine(left=True)
                sns.move_legend(ax, "lower right", bbox_to_anchor=(1, 1), frameon=False, title=None, ncol=3)
                
                charts.append({"title": f"KYC: {kyc_col}", "img_b64": fig_to_base64(fig)})
        except: pass
    ...
    # =====================================================
    # GARANTIA DE COLUNAS CRÍTICAS (SEGURANÇA)
    # =====================================================
    if 'game_name' not in df.columns:
        df['game_name'] = 'Desconhecido'
...
# -----------------------------
# Helpers de erro Perplexity
# -----------------------------


def clean_pplx_error(e: Exception) -> str:
    msg = str(e)
    low = msg.lower()

    if "401" in low or "authorization required" in low:
        return (
            "Perplexity retornou 401 (não autorizado). Verifique se a PPLX_API_KEY é válida "
            "e se sua conta tem acesso à API."
        )
    if "<html" in low or "cloudflare" in low or "openresty" in low:
        return (
            "Perplexity retornou uma página HTML em vez de JSON. "
...
# -----------------------------
# IA (Perplexity – resumo EXECUTIVO)
# -----------------------------

def perplexity_summary(
...
"""
    ...
    resp = chatgpt_client.chat.completions.create(
        model=cfg.get("perplexity", {}).get("model", "gpt-4.1-mini"),
        messages=[
...
# -----------------------------
# IA (Perplexity – OCORRÊNCIAS)   até aqui
# -----------------------------


def perplexity_occurrences(
    chatgpt_client: OpenAI,
    cfg: dict,
    nome_arquivo: str,
    process_type: str,
...
"""
    ...
    resp = chatgpt_client.chat.completions.create(
        ...
        temperature=0.2,
        max_tokens=cfg.get("perplexity", {}).get("max_output_tokens", 600),
    )
...
{% for f in files %}
  <div class="card">
    ...
    <h3>Resumo IA (Principal – Perplexity)</h3>
    <details open>
...
def build_perplexity_client(cfg, logger):
    ...
    base_url = cfg.get("perplexity", {}).get("base_url", "https://api.openai.com/v1")
...
# ... (dentro de analyze_file)
    df = normalize_cols(df)
    summary = summarize_dataframe(df) # Linha existente

    # --- ADICIONE ISTO AQUI ---
    universal_stats = analise_universal_agnostica(df)
    # --------------------------

    filename = os.path.basename(path)
    proc_type = guess_process_type(filename, df)
    alerts, insights = detect_alerts_and_insights(df, cfg, filename) # Linha existente

    # --- ADICIONE ISTO AQUI ---
    insights.update(universal_stats) # Mistura a inteligência nova com a velha
    # --------------------------
    ...
    if chatgpt_client:
        try:
            ...
            ai_summary = f"Falha ao gerar resumo IA (Perplexity): {clean_pplx_error(e)}"

        try:
            occurrences_raw = perplexity_occurrences(
                chatgpt_client=chatgpt_client,
                cfg=cfg,
            ...
            occurrences_raw = {
                ...
                "resumo_geral": f"Falha ao gerar mapa de ocorrências: {clean_pplx_error(e)}",
                "ocorrencias": [],
            }
    else:
        ai_summary = "Perplexity desativado ou indisponível. Sem resumo IA principal."
        occurrences_raw = {
            "arquivo": filename,
            "tipo_processo": proc_type,
            "resumo_geral": "Perplexity desativado/indisponível. Sem mapa de ocorrências detalhado.",
            "ocorrencias": [],
        }
    ...
    if chat_client:
        try:
            resp = chat_client.chat.completions.create(
                ...
                max_tokens=cfg["perplexity"].get("max_output_tokens", 700),
                temperature=0.2,
            )
            ...
            ai_summary = f"Falha ao gerar protocolo IA (imagem): {clean_pplx_error(e)}"
        ...
        ai_summary = "Perplexity desativado ou indisponível. Sem protocolo IA para imagem."
        occurrences_raw = {
            "arquivo": filename,
            "tipo_processo": proc_type,
...
"""
            ...
            resp = chatgpt_client.chat.completions.create(
                ...
                temperature=0.25,
                max_tokens=cfg.get("perplexity", {}).get("max_output_tokens", 700),
            )
            text = resp.choices[0].message.content
            agg["ia_summary"] = text.strip() if isinstance(text, str) else str(text)
        else:
            agg["ia_summary"] = "Perplexity desativado/indisponível. Sem resumo diário IA."
    except Exception as e:
        logger.warning("Falha ao gerar resumo diário IA: %s", e)
        agg["ia_summary"] = f"Falha ao gerar resumo diário IA: {clean_pplx_error(e)}"
...
def main():
    ...
    chat_client = None
    try:
        chat_client = build_perplexity_client(cfg, logger)
    except Exception as e:
